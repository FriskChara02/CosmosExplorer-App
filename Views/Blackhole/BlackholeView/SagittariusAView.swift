...
struct SagittariusAView: View {
    ...
    @StateObject private var commentVM = BlackholeViewModel.BlackholeCommentViewModel()
    @EnvironmentObject var authVM: AuthViewModel
    private var blackholeId: UUID { blackhole.id }
    @Environment(\.modelContext) private var modelContext
    
    private let infoItems: [String] = [
        LanguageManager.current.string("Sagittarius A Random Info 1"),
        LanguageManager.current.string("Sagittarius A Random Info 2"),
        LanguageManager.current.string("Sagittarius A Random Info 3"),
        LanguageManager.current.string("Sagittarius A Random Info 4"),
        LanguageManager.current.string("Sagittarius A Random Info 5")
    ]
        ...
        NavigationStack {
            VStack {
                ...
                HStack {
                    ...
                    Spacer()
                    Text("\(LanguageManager.current.string(selectedTab)) Blackhole")
                        .font(.title)
                        .fontWeight(.bold)
                        .foregroundColor(.white)
                    Spacer()
                    Button(action: {
                        if selectedTab == "Wiki" {
                            UIPasteboard.general.string = "https://en.wikipedia.org/wiki/Sagittarius_A*"
                        } else {
                            withAnimation(.spring()) {
                                isFavorite.toggle()
                                blackhole.isFavorite = isFavorite
                ...
                TabBarSagittariusAView(selectedTab: $selectedTab)
                
                // Nội dung tab
                VStack {
                    if selectedTab == "Overview" {
                        OverviewSagittariusAView(
                            blackhole: blackhole,
                        ...
                        GalleriesSagittariusAView(animation: animation)
                    } else if selectedTab == "Comment" {
                        CommentSagittariusAView(blackholeId: blackholeId).environmentObject(commentVM)
                    } else if selectedTab == "Wiki" {
                        VStack {
                            Text(LanguageManager.current.string("Wikipedia: Sagittarius A"))
                                .font(.headline)
                                .foregroundColor(.white)
                                .padding()
                            WebView(url: URL(string: "https://en.wikipedia.org/wiki/Sagittarius_A*")!)
            ...
            .background(Image("SagittariusA_background").resizable().scaledToFill().ignoresSafeArea())
            .animation(.easeInOut(duration: 1.0), value: glowIntensity)
            .onAppear {
...
struct TabBarSagittariusAView: View {
...
struct OverviewSagittariusAView: View {
    let blackhole: BlackholeModel
    ...
    var body: some View {
        ZStack {
            Image("SagittariusA")
                ...
                .padding()
                .matchedGeometryEffect(id: "sagittariusa-\(blackhole.id)", in: animation)
...
struct InformationSagittariusAView: View {
    let blackhole: BlackholeModel
    @Binding var glowIntensity: Float
            ...
            VStack {
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 16) {
                        ...
                        Text(blackhole.aboutDescription.isEmpty ? LanguageManager.current.string("Sagittarius A About Description") : blackhole.aboutDescription)
                            .font(.body)
                            .foregroundColor(.white)
                            .padding(.horizontal)
                            .transition(.opacity)
                        ...
                        HStack {
                            ...
                            Text(LanguageManager.current.string("Video"))
                                ...
                                .foregroundColor(.white)
                            Spacer()
                            NavigationLink(destination: VideoListSagittariusAView(videoURLs: blackhole.videoURLs)) {
                        ...
                        LazyVGrid(columns: [
                            ...
                            ForEach(blackhole.videoURLs.isEmpty ? [
                                "https://www.youtube.com/embed/Zml0dZCjaFw",
                                "https://www.youtube.com/embed/ChJU3pWRcrA"
                            ] : blackhole.videoURLs, id: \.self) { url in
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            infoRow(icon: "circle.fill", color: .orange, text: "\(LanguageManager.current.string("Radius")): \(blackhole.radius.isEmpty ? "Unknown" : blackhole.radius)")
                            infoRow(icon: "sun.max.fill", color: .red, text: "\(LanguageManager.current.string("Distance from Sun")): \(blackhole.distanceFromSun.isEmpty ? "Unknown" : blackhole.distanceFromSun)")
                            infoRow(icon: "hourglass", color: .teal, text: "\(LanguageManager.current.string("Age")): \(blackhole.age.isEmpty ? "Unknown" : blackhole.age)")
                        }
                        .padding(.horizontal)
                    }
                    .padding(.vertical)
...
struct VideoListSagittariusAView: View {
        ...
        ScrollView {
            LazyVGrid(columns: [
                ...
                ForEach(videoURLs.isEmpty ? [
                    "https://www.youtube.com/embed/XA7CAVm31z0",
                    "https://www.youtube.com/embed/Zml0dZCjaFw",
                    "https://www.youtube.com/embed/mnrktWsiTx8",
                    "https://www.youtube.com/embed/ChJU3pWRcrA"
...
struct GalleriesSagittariusAView: View {
    let animation: Namespace.ID
    let images = ["SagittariusA", "SagittariusA01", "SagittariusA02", "SagittariusA03", "SagittariusA04", "SagittariusA05"]
...
struct CommentSagittariusAView: View {
    @EnvironmentObject var vm: BlackholeViewModel.BlackholeCommentViewModel
    @EnvironmentObject var authVM: AuthViewModel
    
    let blackholeId: UUID
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            CommentInputBar(text: $messageText) {
                vm.sendComment(blackholeId: blackholeId, content: messageText)
                messageText = ""
            }
        }
        .onAppear {
            vm.loadComments(for: blackholeId)
        }
...
#Preview {
    SagittariusAView(
        blackhole: BlackholeModel(name: "Sagittarius A", blackholeDescription: "A supermassive black hole.", blackhole_order: 0),
