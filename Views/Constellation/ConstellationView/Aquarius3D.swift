...
//  CosmosExplorer
//
//  Created by Loi Nguyen on 22/10/25.
//

import SwiftUI
import RealityKit
struct AquariusOverview3D: View {
    ...
    var body: some View {
        RealityView { content in
            ...
            let starOffsets: [CGPoint] = [
                CGPoint(x: -0.4, y: 0.0),   // Star 1
                CGPoint(x: -0.3, y: 0.19),  // Star 2
                CGPoint(x: -0.4, y: -0.3),  // Star 3
                CGPoint(x: -0.3, y: -0.2),  // Star 4
                CGPoint(x: -0.2, y: -0.0),  // Star 5
                CGPoint(x:  0.0, y: 0.3),   // Star 6 ⭐️ (tâm)
                CGPoint(x:  0.1, y: 0.28),  // Star 7
                CGPoint(x:  0.2, y: 0.18),  // Star 8
                CGPoint(x:  0.35, y: 0.08), // Star 9
                CGPoint(x:  0.53, y: -0.20) // Star 10
            ]
            ...
            let connections: [(Int, Int)] = [
                (0, 1), (2, 3), (3, 4), (1, 5), (4, 5), (5, 6), (6, 7), (7, 8), (8, 9)
            ]
...
struct AquariusInformation3D: View {
    @State private var constellationEntity = Entity()
    
    var body: some View {
        RealityView { content in
            // ✨ VỊ TRÍ SAO THỰC TẾ AQUARIUS
            
            let starOffsets: [CGPoint] = [
                CGPoint(x: -0.4, y: 0.0),   // Sadalsuud (β Aqr)
                CGPoint(x: -0.3, y: 0.19),  // Sadalmelik (α Aqr)
                CGPoint(x: -0.4, y: -0.3),  // Sadachbia (γ Aqr)
                CGPoint(x: -0.3, y: -0.2),  // Skat (δ Aqr)
                CGPoint(x: -0.2, y: -0.0),  // Ancha (θ Aqr)
                CGPoint(x:  0.0, y: 0.3),   // Albali (ε Aqr)
                CGPoint(x:  0.1, y: 0.28),  // Bunda (ξ Aqr)
                CGPoint(x:  0.2, y: 0.18),  // Situla (κ Aqr)
                CGPoint(x:  0.35, y: 0.08), // Hydor (λ Aqr)
                CGPoint(x:  0.53, y: -0.20) // Phong (ψ Aqr)
            ]
