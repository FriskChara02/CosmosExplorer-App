...
struct CancerCView: View {
    ...
    private let infoItems: [String] = [
        LanguageManager.current.string("Cancer Random Info 1"),
        LanguageManager.current.string("Cancer Random Info 2"),
        LanguageManager.current.string("Cancer Random Info 3"),
        LanguageManager.current.string("Cancer Random Info 4"),
        LanguageManager.current.string("Cancer Random Info 5")
    ]
        ...
        NavigationStack {
            VStack {
                ...
                HStack {
                    ...
                    Spacer()
                    Text("\(LanguageManager.current.string(selectedTab)) Cancer")
                        .font(.title)
                        .fontWeight(.bold)
                        .foregroundColor(.white)
                    Spacer()
                    Button(action: {
                        if selectedTab == LanguageManager.current.string("Wiki") {
                            UIPasteboard.general.string = "https://en.wikipedia.org/wiki/Cancer_(constellation)"
                        } else {
                ...
                .padding(.horizontal)
                .padding(.top)
                TabBarCancerView(selectedTab: $selectedTab)
                // Nội dung tab
                VStack {
                    if selectedTab == LanguageManager.current.string("Overview") {
                        ...
                        GalleriesCancerView(animation: animation)
                    } else if selectedTab == LanguageManager.current.string("Comment") {
                        CommentCancerView(constellationId: constellationId).environmentObject(commentVM)
                    } else if selectedTab == LanguageManager.current.string("Wiki") {
                        VStack {
                            Text(LanguageManager.current.string("Wikipedia: Cancer Constellation"))
                                .font(.headline)
                                .foregroundColor(.white)
                                .padding()
                            WebView(url: URL(string: "https://en.wikipedia.org/wiki/Cancer_(constellation)")!)
            ...
            .background(Image("Cancer_background").resizable().scaledToFill().ignoresSafeArea().overlay(Color.black.opacity(0.4)))
            .animation(.easeInOut(duration: 1.0), value: glowIntensity)
            .onAppear {
...
struct OverviewCancerView: View {
    let constellation: ConstellationModel
    ...
    var body: some View {
        ZStack {
            CancerOverview3D()
                .frame(height: 500)
                .padding()
                .matchedGeometryEffect(id: "cancer-\(constellation.id)", in: animation)
            VStack {
                Image("CancerSymbol")
                    .resizable()
                    .scaledToFit()
...
struct InformationCancerView: View {
    let constellation: ConstellationModel
    @Binding var glowIntensity: Float
            ...
            VStack {
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 16) {
                        ...
                        .padding(.horizontal)
                        Text(constellation.aboutDescription.isEmpty ? LanguageManager.current.string("Cancer About Description") : constellation.aboutDescription)
                            .font(.body)
                            .foregroundColor(.white)
                            .padding(.horizontal)
                            .transition(.opacity)
                        ...
                        HStack {
                            ...
                            Text(LanguageManager.current.string("Video"))
                                ...
                                .foregroundColor(.white)
                            Spacer()
                            NavigationLink(destination: VideoListCancerView(videoURLs: constellation.videoURLs)) {
                        ...
                        LazyVGrid(columns: [
                            ...
                            ForEach(constellation.videoURLs.isEmpty ? [
                                "https://www.youtube.com/embed/WIzBwAmloYQ",
                                "https://www.youtube.com/embed/Ae_a2j2YJY0"
...
struct VideoListCancerView: View {
    ...
    var body: some View {
        ScrollView {
            LazyVGrid(columns: [
                ...
                ForEach(videoURLs.isEmpty ? [
                    "https://www.youtube.com/embed/WIzBwAmloYQ",
                    "https://www.youtube.com/embed/Ae_a2j2YJY0",
                    "https://www.youtube.com/embed/NOYuTaKFimk",
                    "https://www.youtube.com/embed/tUKDvPb5D_I"
        ...
        .background(Image("Cancer_background").resizable().scaledToFill().ignoresSafeArea())
        .navigationTitle(LanguageManager.current.string("Videos"))
        .navigationBarTitleDisplayMode(.inline)
    }
}

struct GalleriesCancerView: View {
    let animation: Namespace.ID
    let images = ["Cancer", "Cancer01", "Cancer02", "Cancer03", "Cancer04"]
    var body: some View {
        ZStack {
            LinearGradient(
...
#Preview {
    CancerCView(
        constellation: ConstellationModel(name: "Cancer", constellationDescription: "The crab.", constellation_order: 1, namedStars: ["Tarf", "Acubens", "Asellus Borealis", "Asellus Australis", "Altarf"]),
