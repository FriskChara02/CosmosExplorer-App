...
struct CapricornCView: View {
    ...
    private let infoItems: [String] = [
        LanguageManager.current.string("Capricorn Random Info 1"),
        LanguageManager.current.string("Capricorn Random Info 2"),
        LanguageManager.current.string("Capricorn Random Info 3"),
        LanguageManager.current.string("Capricorn Random Info 4"),
        LanguageManager.current.string("Capricorn Random Info 5")
    ]
        ...
        NavigationStack {
            VStack {
                ...
                HStack {
                    ...
                    Spacer()
                    Text("\(LanguageManager.current.string(selectedTab)) Capricorn")
                        .font(.title)
                        .fontWeight(.bold)
                        .foregroundColor(.white)
                    Spacer()
                    Button(action: {
                        if selectedTab == LanguageManager.current.string("Wiki") {
                            UIPasteboard.general.string = "https://en.wikipedia.org/wiki/Capricornus"
                        } else {
                ...
                .padding(.horizontal)
                .padding(.top)
                TabBarCapricornView(selectedTab: $selectedTab)
                // Nội dung tab
                VStack {
                    if selectedTab == LanguageManager.current.string("Overview") {
                        OverviewCapricornView(
                        ...
                        GalleriesCapricornView(animation: animation)
                    } else if selectedTab == LanguageManager.current.string("Comment") {
                        CommentCapricornView(constellationId: constellationId).environmentObject(commentVM)
                    } else if selectedTab == LanguageManager.current.string("Wiki") {
                        VStack {
                            Text(LanguageManager.current.string("Wikipedia: Capricorn Constellation"))
                                .font(.headline)
                                .foregroundColor(.white)
                                .padding()
                            WebView(url: URL(string: "https://en.wikipedia.org/wiki/Capricornus")!)
            ...
            .background(Image("Capricornus_background").resizable().scaledToFill().ignoresSafeArea().overlay(Color.black.opacity(0.4)))
            .animation(.easeInOut(duration: 1.0), value: glowIntensity)
            .onAppear {
...
struct TabBarCapricornView: View {
...
struct OverviewCapricornView: View {
    let constellation: ConstellationModel
    ...
    var body: some View {
        ZStack {
            CapricornOverview3D()
                .frame(height: 500)
                .padding()
                .matchedGeometryEffect(id: "capricorn-\(constellation.id)", in: animation)
            VStack {
                Image("CapricornSymbol")
                    .resizable()
                    .scaledToFit()
...
struct InformationCapricornView: View {
    let constellation: ConstellationModel
    @Binding var glowIntensity: Float
            ...
            VStack {
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 16) {
                        ...
                        .padding(.horizontal)
                        Text(constellation.aboutDescription.isEmpty ? LanguageManager.current.string("Capricorn About Description") : constellation.aboutDescription)
                            .font(.body)
                            .foregroundColor(.white)
                            .padding(.horizontal)
                            .transition(.opacity)
                        ...
                        HStack {
                            ...
                            Text(LanguageManager.current.string("Video"))
                                ...
                                .foregroundColor(.white)
                            Spacer()
                            NavigationLink(destination: VideoListCapricornView(videoURLs: constellation.videoURLs)) {
                        ...
                        LazyVGrid(columns: [
                            ...
                            ForEach(constellation.videoURLs.isEmpty ? [
                                "https://www.youtube.com/embed/uDvvKm99mjU",
                                "https://www.youtube.com/embed/hr1HvgOXsdM"
...
struct VideoListCapricornView: View {
    ...
    var body: some View {
        ScrollView {
            LazyVGrid(columns: [
                ...
                ForEach(videoURLs.isEmpty ? [
                    "https://www.youtube.com/embed/uDvvKm99mjU",
                    "https://www.youtube.com/embed/hr1HvgOXsdM",
                    "https://www.youtube.com/embed/G22TqEovxEQ",
                    "https://www.youtube.com/embed/ogyJe666L9M"
        ...
        .background(Image("Capricornus_background").resizable().scaledToFill().ignoresSafeArea())
        .navigationTitle(LanguageManager.current.string("Videos"))
        .navigationBarTitleDisplayMode(.inline)
    }
}

struct GalleriesCapricornView: View {
    let animation: Namespace.ID
    let images = ["Capricorn", "Capricorn01", "Capricorn02", "Capricorn03", "Capricorn04"]
    var body: some View {
        ZStack {
            LinearGradient(
...
struct CommentCapricornView: View {
...
#Preview {
    CapricornCView(
        constellation: ConstellationModel(name: "Capricorn", constellationDescription: "The sea goat.", constellation_order: 2, namedStars: ["Deneb Algedi", "Dabih", "Nashira", "Algedi", "Alshat"]),
