...
struct BlastCatalogView: View {
    @StateObject private var viewModel = QuizListViewModel(service: SwiftDataService(), mode: "Blast")
    ...
    @State private var ownBlastLimit: Int = 6
    @State private var othersBlastLimit: Int = 6
            ...
            VStack {
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 20) {
                        // App Samples
                        Section(header: Text("App's Sample Blast Sets").font(.headline).foregroundColor(.white)) {
                            LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 16) {
                                ForEach(viewModel.filteredQuizzes.filter { $0.createdBy == nil }.prefix(appSamplesLimit)) { quiz in
                                    BlastItemView(quiz: quiz, viewModel: viewModel, editingQuiz: $editingQuiz)
                                }
                            }
                        ...
                        // Your Blast Sets
                        Section(header: Text("Your Blast Sets").font(.headline).foregroundColor(.white)) {
                            LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 16) {
                                ForEach(viewModel.filteredQuizzes.filter { $0.createdBy == viewModel.currentUserId }.prefix(ownBlastLimit)) { quiz in
                                    BlastItemView(quiz: quiz, viewModel: viewModel, editingQuiz: $editingQuiz)
                                }
                            }
                            if viewModel.filteredQuizzes.filter({ $0.createdBy == viewModel.currentUserId }).count > ownBlastLimit {
                                Button("See more") {
                                    ownBlastLimit += 6
                                }
                                .foregroundColor(.blue)
                        ...
                        // Community
                        Section(header: Text("Community Blast Sets").font(.headline).foregroundColor(.white)) {
                            LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 16) {
                                ForEach(viewModel.filteredQuizzes.filter { $0.createdBy != nil && $0.createdBy != viewModel.currentUserId && $0.isPublic }.prefix(othersBlastLimit)) { quiz in
                                    BlastItemView(quiz: quiz, viewModel: viewModel, editingQuiz: $editingQuiz)
                                }
                            }
                            if viewModel.filteredQuizzes.filter({ $0.createdBy != nil && $0.createdBy != viewModel.currentUserId && $0.isPublic }).count > othersBlastLimit {
                                Button("See more") {
                                    othersBlastLimit += 6
                                }
                                .foregroundColor(.blue)
...
struct BlastItemView: View {
    ...
    var body: some View {
        NavigationLink(destination: BlastView(quiz: quiz, service: viewModel.service)) {
            ZStack {
