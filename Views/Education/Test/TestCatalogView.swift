...
struct TestCatalogView: View {
    @StateObject private var viewModel = QuizListViewModel(service: SwiftDataService(), mode: "Test")
    ...
    @State private var ownTestsLimit: Int = 6
    @State private var othersTestsLimit: Int = 6
            ...
            VStack {
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 20) {
                        // App Samples
                        Section(header: Text("App's Sample Tests").font(.headline).foregroundColor(.white)) {
                            LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 16) {
                                ForEach(viewModel.filteredQuizzes.filter { $0.createdBy == nil }.prefix(appSamplesLimit)) { quiz in
                                    TestItemView(quiz: quiz, viewModel: viewModel, editingQuiz: $editingQuiz)
                                }
                            }
                        ...
                        // Your Tests
                        Section(header: Text("Your Tests").font(.headline).foregroundColor(.white)) {
                            LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 16) {
                                ForEach(viewModel.filteredQuizzes.filter { $0.createdBy == viewModel.currentUserId }.prefix(ownTestsLimit)) { quiz in
                                    TestItemView(quiz: quiz, viewModel: viewModel, editingQuiz: $editingQuiz)
                                }
                            }
                            if viewModel.filteredQuizzes.filter({ $0.createdBy == viewModel.currentUserId }).count > ownTestsLimit {
                                Button("See more") {
                                    ownTestsLimit += 6
                                }
                                .foregroundColor(.blue)
                        ...
                        // Community Tests
                        Section(header: Text("Community Tests").font(.headline).foregroundColor(.white)) {
                            LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 16) {
                                ForEach(viewModel.filteredQuizzes.filter { $0.createdBy != nil && $0.createdBy != viewModel.currentUserId && $0.isPublic }.prefix(othersTestsLimit)) { quiz in
                                    TestItemView(quiz: quiz, viewModel: viewModel, editingQuiz: $editingQuiz)
                                }
                            }
                            if viewModel.filteredQuizzes.filter({ $0.createdBy != nil && $0.createdBy != viewModel.currentUserId && $0.isPublic }).count > othersTestsLimit {
                                Button("See more") {
                                    othersTestsLimit += 6
                                }
                                .foregroundColor(.blue)
...
struct TestItemView: View {
    ...
    var body: some View {
        NavigationLink(destination: TestView(quiz: quiz, service: viewModel.service)) {
            ZStack {
