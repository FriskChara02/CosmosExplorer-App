...
struct GalaxyView: View {
        ...
        NavigationStack {
            VStack {
                ...
                HStack {
                    ...
                    Spacer()
                    Text("\(LanguageManager.current.string(selectedTab)) \(LanguageManager.current.string("Galaxy"))")
                        .font(.title)
                        .fontWeight(.bold)
                        .foregroundColor(.white)
                    Spacer()
                    Button(action: {
                        if selectedTab == LanguageManager.current.string("Wiki") {
                            UIPasteboard.general.string = galaxy.wikiLink.isEmpty ? "https://en.wikipedia.org/wiki/\(galaxy.name)" : galaxy.wikiLink
                        } else {
                    ...
                    if selectedTab == LanguageManager.current.string("Overview") {
                        ...
                        VStack {
                            Text("\(LanguageManager.current.string("Wiki")): \(galaxy.name)")
                                ...
                                .foregroundColor(.white)
                                .padding()
                            WebView(url: URL(string: galaxy.wikiLink.isEmpty ? "https://en.wikipedia.org/wiki/\(galaxy.name)" : galaxy.wikiLink)!)
    ...
    private func updateRandomInfo() {
        randomInfo = galaxy.randomInfos.randomElement() ?? ""
    }
            ...
            ZStack {
                if let data = galaxy.imageData, let uiImage = UIImage(data: data) {
                    Image(uiImage: uiImage)
                        ...
                        .padding()
                        .matchedGeometryEffect(id: "galaxy-\(galaxy.id)", in: animation)
                } else {
                ...
                VStack {
                    ...
                    ScrollView {
                        VStack(alignment: .leading, spacing: 16) {
                            ...
                            Text(galaxy.aboutDescription.isEmpty ? LanguageManager.current.string("No description available") : galaxy.aboutDescription)
                                .font(.body)
                                .foregroundColor(.white)
                                .padding(.horizontal)
                                .transition(.opacity)
                            ...
                            HStack {
                                ...
                                Text(LanguageManager.current.string("Video"))
                                    ...
                                    .foregroundColor(.white)
                                Spacer()
                                NavigationLink(destination: VideoListView(videoURLs: galaxy.videoURLs, galaxyName: galaxy.name)) {
                            ...
                            LazyVGrid(columns: [
                                ...
                                ForEach(galaxy.videoURLs.prefix(2), id: \.self) { url in
                            ...
                            VStack(alignment: .leading, spacing: 8) {
                                infoRow(icon: "circle.fill", color: .orange, text: "\(LanguageManager.current.string("Radius")): \(galaxy.radius.isEmpty ? LanguageManager.current.string("Unknown") : galaxy.radius)")
                                infoRow(icon: "sun.max.fill", color: .red, text: "\(LanguageManager.current.string("Distance from Sun")): \(galaxy.distanceFromSun.isEmpty ? LanguageManager.current.string("Unknown") : galaxy.distanceFromSun)")
                                infoRow(icon: "hourglass", color: .teal, text: "\(LanguageManager.current.string("Age")): \(galaxy.age.isEmpty ? LanguageManager.current.string("Unknown") : galaxy.age)")
                            }
                            .padding(.horizontal)
                        }
                        .padding(.vertical)
    ...
    struct VideoListView: View {
        ...
        init(videoURLs: [String], galaxyName: String = "") {
        ...
        var body: some View {
            ...
            .onAppear {
                if !galaxyName.isEmpty {
                    print("📱 \(LanguageManager.current.string("VideoListView appeared for")) \(galaxyName)")
                }
            }
        }
    }
            ...
            ZStack {
                ...
                ScrollView {
                    LazyVGrid(columns: [
                        ...
                        ForEach(galaxy.galleryImageData.indices, id: \.self) { index in
                            if let data = galaxy.galleryImageData[index], let uiImage = UIImage(data: data) {
...
#Preview {
    GalaxyView(
        galaxy: GalaxyModel(name: "Test Galaxy", galaxyDescription: "Test", galaxy_order: 0),
