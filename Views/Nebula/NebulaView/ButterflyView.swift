...
struct ButterflyView: View {
    ...
    @StateObject private var commentVM = NebulaViewModel.NebulaCommentViewModel()
    @EnvironmentObject var authVM: AuthViewModel
    private var nebulaId: UUID { nebula.id }
    @Environment(\.modelContext) private var modelContext

    private let infoItems: [String] = [
        LanguageManager.current.string("Butterfly Random Info 1"),
        LanguageManager.current.string("Butterfly Random Info 2"),
        LanguageManager.current.string("Butterfly Random Info 3"),
        LanguageManager.current.string("Butterfly Random Info 4"),
        LanguageManager.current.string("Butterfly Random Info 5")
    ]
    ...
    var body: some View {
        NavigationStack {
            VStack {
                ...
                HStack {
                    ...
                    Button(action: {
                        if selectedTab == LanguageManager.current.string("Wiki") {
                            UIPasteboard.general.string = "https://en.wikipedia.org/wiki/Butterfly_Nebula"
                        } else {
                ...
                TabBarButterflyView(selectedTab: $selectedTab)

                // Nội dung tab
                VStack {
                    if selectedTab == LanguageManager.current.string("Overview") {
                        OverviewButterflyView(
                            nebula: nebula,
                        ...
                        GalleriesButterflyView(animation: animation)
                    } else if selectedTab == LanguageManager.current.string("Comment") {
                        CommentButterflyView(nebulaId: nebulaId).environmentObject(commentVM)
                    } else if selectedTab == LanguageManager.current.string("Wiki") {
                        VStack {
                            Text(LanguageManager.current.string("Wikipedia: Butterfly Nebula"))
                                .font(.headline)
                                .foregroundColor(.white)
                                .padding()
                            WebView(url: URL(string: "https://en.wikipedia.org/wiki/NGC_6302")!)
            ...
            .background(Image("Butterfly_background").resizable().scaledToFill().ignoresSafeArea().overlay(Color.black.opacity(0.4)))
            .animation(.easeInOut(duration: 1.0), value: glowIntensity)
            .onAppear {
...
struct TabBarButterflyView: View {
...
struct OverviewButterflyView: View {
    let nebula: NebulaModel
    ...
    var body: some View {
        ZStack {
            Image("Butterfly")
                ...
                .padding()
                .matchedGeometryEffect(id: "butterfly-\(nebula.id)", in: animation)
...
struct InformationButterflyView: View {
    let nebula: NebulaModel
    @Binding var glowIntensity: Float
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 16) {
                        HStack {
                            ...
                            Button(action: {
                                if selectedTab == LanguageManager.current.string("Wiki") {
                                    UIPasteboard.general.string = "https://en.wikipedia.org/wiki/NGC_6302"
                                } else {
                                    withAnimation(.spring()) {
                                        isFavorite.toggle()
                                        viewModel.toggleFavorite(nebula: nebula)
                        ...
                        Text(nebula.aboutDescription.isEmpty ? LanguageManager.current.string("Butterfly About Description") : nebula.aboutDescription)
                            .font(.body)
                            .foregroundColor(.white)
                            .padding(.horizontal)
                            .transition(.opacity)
                        ...
                        HStack {
                            ...
                            Text(LanguageManager.current.string("Video"))
                                ...
                                .foregroundColor(.white)
                            Spacer()
                            NavigationLink(destination: VideoListButterflyView(videoURLs: nebula.videoURLs)) {
                        ...
                        LazyVGrid(columns: [
                            ...
                            ForEach(nebula.videoURLs.isEmpty ? [
                                "https://www.youtube.com/embed/She1qqoiqW4",
                                "https://www.youtube.com/embed/wEJuAaH65eY"
...
struct VideoListButterflyView: View {
    ...
    var body: some View {
        ScrollView {
            LazyVGrid(columns: [
                ...
                ForEach(videoURLs.isEmpty ? [
                    "https://www.youtube.com/embed/She1qqoiqW4",
                    "https://www.youtube.com/embed/wEJuAaH65eY",
                    "https://www.youtube.com/embed/KypIdOMrL10",
                    "https://www.youtube.com/embed/WCNbKIC1_E8"
        ...
        .background(Image("Butterfly_background").resizable().scaledToFill().ignoresSafeArea())
        .navigationTitle(LanguageManager.current.string("Videos"))
        .navigationBarTitleDisplayMode(.inline)
    }
}

struct GalleriesButterflyView: View {
    let animation: Namespace.ID
    let images = ["Butterfly_background", "Butterfly01", "Butterfly02", "Butterfly03", "Butterfly04", "Butterfly05", "Butterfly06"]
...
struct CommentButterflyView: View {
    @EnvironmentObject var vm: NebulaViewModel.NebulaCommentViewModel
    @EnvironmentObject var authVM: AuthViewModel
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            CommentInputBar(text: $messageText) {
                vm.sendComment(nebulaId: nebulaId, content: messageText)
                messageText = ""
            }
        }
        .onAppear {
            vm.loadComments(for: nebulaId)
        }
...
#Preview {
    ButterflyView(
        nebula: NebulaModel(name: "Butterfly", nebulaDescription: "The Butterfly Nebula.", nebula_order: 1),
