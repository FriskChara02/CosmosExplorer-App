...
struct PlanetsView: View {
    ...
    @StateObject private var commentVM = PlanetsViewModel.PlanetsCommentViewModel()
    @EnvironmentObject var authVM: AuthViewModel
    private var planetsId: UUID { planets.id }
            ...
            VStack {
                ...
                HStack {
                    ...
                    Button(action: {
                        if selectedTab == LanguageManager.current.string("Wiki") {
                            UIPasteboard.general.string = planets.wikiLink.isEmpty ? "https://en.wikipedia.org/wiki/\(planets.name)" : planets.wikiLink
                        } else {
                ...
                VStack {
                    if selectedTab == LanguageManager.current.string("Overview") {
                        ...
                        GalleriesView(planets: planets, animation: animation)
                    } else if selectedTab == LanguageManager.current.string("Comment") {
                        CommentView(planetsId: planetsId).environmentObject(commentVM)
                    } else if selectedTab == LanguageManager.current.string("Wiki") {
                        VStack {
                            Text("\(LanguageManager.current.string("Wiki")): \(planets.name)")
                                .font(.headline)
                                .foregroundColor(.white)
                                .padding()
                            WebView(url: URL(string: planets.wikiLink.isEmpty ? "https://en.wikipedia.org/wiki/\(planets.name)" : planets.wikiLink)!)
    ...
    private func updateRandomInfo() {
        randomInfo = planets.randomInfos.randomElement() ?? ""
    }
            ...
            ZStack {
                if let data = planets.imageData, let uiImage = UIImage(data: data) {
                    Image(uiImage: uiImage)
                        ...
                        .padding()
                        .matchedGeometryEffect(id: "planets-\(planets.id)", in: animation)
                } else {
                ...
                VStack {
                    ...
                    ScrollView {
                        VStack(alignment: .leading, spacing: 16) {
                            ...
                            .padding(.horizontal)
                            Text(planets.aboutDescription.isEmpty ? LanguageManager.current.string("No description available") : planets.aboutDescription)
                                .font(.body)
                                .foregroundColor(.white)
                                .padding(.horizontal)
                                .transition(.opacity)
                            ...
                            HStack {
                                ...
                                Text(LanguageManager.current.string("Video"))
                                    ...
                                    .foregroundColor(.white)
                                Spacer()
                                NavigationLink(destination: VideoListView(videoURLs: planets.videoURLs, planetsName: planets.name)) {
                            ...
                            LazyVGrid(columns: [
                                ...
                                ForEach(planets.videoURLs.prefix(2), id: \.self) { url in
                            ...
                            VStack(alignment: .leading, spacing: 8) {
                                infoRow(icon: "circle.fill", color: .orange, text: "\(LanguageManager.current.string("Radius")): \(planets.radius.isEmpty ? LanguageManager.current.string("Unknown") : planets.radius)")
                                infoRow(icon: "sun.max.fill", color: .red, text: "\(LanguageManager.current.string("Distance from Sun")): \(planets.distanceFromSun.isEmpty ? LanguageManager.current.string("Unknown") : planets.distanceFromSun)")
                                infoRow(icon: "hourglass", color: .teal, text: "\(LanguageManager.current.string("Age")): \(planets.age.isEmpty ? LanguageManager.current.string("Unknown") : planets.age)")
                            }
                            .padding(.horizontal)
                        }
                        .padding(.vertical)
    ...
    struct VideoListView: View {
        ...
        let planetsName: String
        
        init(videoURLs: [String], planetsName: String = "") {
        ...
        var body: some View {
            ...
            .onAppear {
                if !planetsName.isEmpty {
                    print("📱 \(LanguageManager.current.string("VideoListView appeared for")) \(planetsName)")
                }
            }
        }
    }
            ...
            ZStack {
                ...
                ScrollView {
                    LazyVGrid(columns: [
                        ...
                        ForEach(planets.galleryImageData.indices, id: \.self) { index in
                            if let data = planets.galleryImageData[index], let uiImage = UIImage(data: data) {
    ...
    struct CommentView: View {
        @EnvironmentObject var vm: PlanetsViewModel.PlanetsCommentViewModel
        @EnvironmentObject var authVM: AuthViewModel
        ...
        var body: some View {
            VStack(spacing: 0) {
                ...
                CommentInputBar(text: $messageText) {
                    vm.sendComment(planetsId: planetsId, content: messageText)
                    messageText = ""
                }
            }
            .onAppear {
                vm.loadComments(for: planetsId)
            }
...
#Preview {
    PlanetsView(
        planets: PlanetsModel(name: "Test Planets", planetsDescription: "Test", planets_order: 0),
