...
struct UranusView: View {
    ...
    private let infoItems: [String] = [
        LanguageManager.current.string("Uranus Random Info 1"),
        LanguageManager.current.string("Uranus Random Info 2"),
        LanguageManager.current.string("Uranus Random Info 3"),
        LanguageManager.current.string("Uranus Random Info 4"),
        LanguageManager.current.string("Uranus Random Info 5")
    ]
            ...
            VStack {
                ...
                HStack {
                    ...
                    Button(action: {
                        if selectedTab == "Wiki" {
                            ...
                            UIPasteboard.general.string = "https://en.wikipedia.org/wiki/Uranus"
                        } else {
                ...
                VStack {
                    if selectedTab == "Overview" {
                        ...
                        VStack {
                            Text(LanguageManager.current.string("Wikipedia: Uranus"))
                                .font(.headline)
                                .foregroundColor(.white)
                                .padding()
                            WebView(url: URL(string: "https://en.wikipedia.org/wiki/Uranus")!)
                        ...
                        CommentUranusView(planetId: planetId).environmentObject(commentVM)
                    } else {
        ...
        var body: some View {
            ZStack {
                RealityView { content in
                    let uranusMesh = MeshResource.generateSphere(radius: 1.0)
                    ...
                    uranusMaterial.emissiveIntensity = glowIntensity
                    ...
                    var cloudComponent = ParticleEmitterComponent()
                    cloudComponent.emitterShape = .sphere
                    cloudComponent.emitterShapeSize = SIMD3<Float>(repeating: 1.0)
                    cloudComponent.burstCount = 20
                    cloudComponent.burstCountVariation = 5
                    cloudComponent.speed = 0.1
                    cloudComponent.speedVariation = 0.03
                    cloudComponent.birthLocation = .surface
                    cloudComponent.birthDirection = .normal
                    cloudComponent.timing = .repeating(warmUp: 0.0, emit: .init(duration: 3.0), idle: .init(duration: 2.0))
                    cloudComponent.particlesInheritTransform = true
                    
                    let cloudEntity = ModelEntity()
                    cloudEntity.components.set(cloudComponent)
                    cloudEntity.position = [0, 0, 0]
                    uranusEntity.addChild(cloudEntity)
                    
                    var cloudMaterial = UnlitMaterial()
                    cloudMaterial.color = .init(tint: .white)
                    cloudEntity.model = ModelComponent(mesh: MeshResource.generateSphere(radius: 0.01), materials: [cloudMaterial])
                    
                    let pointLight = PointLight()
                    pointLight.light.intensity = 2000
                    pointLight.light.color = .cyan
                    ...
                    Task { @MainActor in
                        ...
                        while true {
                            ...
                            uranusEntity.transform.rotation = rotationY * rotationX
                ...
                .frame(height: 300)
                .padding()
                .matchedGeometryEffect(id: "uranus-\(planet.id)", in: animation)
                .gesture(
                    DragGesture()
                        .onChanged { value in
                            isDragging = true
    ...
    struct InformationView: View {
        ...
        var body: some View {
            GeometryReader { geometry in
                VStack {
                    RealityView { content in
                        let uranusMesh = MeshResource.generateSphere(radius: 0.5)
                        var uranusMaterial = PhysicallyBasedMaterial()
                        do {
                            let textureResource = try await TextureResource(named: "Uranus_Texture")
                            uranusMaterial.baseColor = .init(texture: .init(textureResource))
                        ...
                        Task { @MainActor in
                            ...
                            while true {
                                ...
                                uranusEntity.transform.rotation = rotation
                                try? await Task.sleep(nanoseconds: 16_666_666)
                            }
                        }
                    }
                    ...
                    ScrollView {
                        VStack(alignment: .leading, spacing: 16) {
                            ...
                            Text(LanguageManager.current.string("Uranus About Description"))
                                .font(.body)
                                .foregroundColor(.white)
                                .padding(.horizontal)
                                .transition(.opacity)
                            ...
                            LazyVGrid(columns: [
                                ...
                                ForEach([
                                    "https://www.youtube.com/embed/Y6m_oliZ89M",
                                    "https://www.youtube.com/embed/m4NXbFOiOGk"
                                ],  id: \.self) { url in
    ...
    struct VideoListView: View {
        let videoURLs = [
            "https://www.youtube.com/embed/Y6m_oliZ89M",
            "https://www.youtube.com/embed/m4NXbFOiOGk",
            "https://www.youtube.com/embed/q7HMd2FyFqk",
            "https://www.youtube.com/embed/92yFEGM37AM",
            "https://www.youtube.com/embed/tCmC0XOKO5Q"
        ]
        ...
        var body: some View {
            ScrollView {
                VStack(alignment: .leading, spacing: 2) {
                    infoRow(icon: "star.fill", color: .cyan,
                            text: LanguageManager.current.string("Uranus By The Numbers Planet Type"))
                    
                    infoRow(icon: "circle.fill", color: .blue,
                            text: LanguageManager.current.string("Uranus By The Numbers Radius"))
                    
                    infoRow(icon: "sun.max.fill", color: .purple,
                            text: LanguageManager.current.string("Uranus By The Numbers Distance"))
                    
                    infoRow(icon: "moon.fill", color: .gray,
                            text: LanguageManager.current.string("Uranus By The Numbers Moons"))
                    
                    infoRow(icon: "gauge", color: .green,
                            text: LanguageManager.current.string("Uranus By The Numbers Gravity"))
                    
                    infoRow(icon: "arrow.triangle.2.circlepath", color: .blue,
                            text: LanguageManager.current.string("Uranus By The Numbers Tilt"))
                    
                    infoRow(icon: "calendar", color: .purple,
                            text: LanguageManager.current.string("Uranus By The Numbers Year"))
                    
                    infoRow(icon: "clock", color: .pink,
                            text: LanguageManager.current.string("Uranus By The Numbers Day"))
                    
                    infoRow(icon: "thermometer", color: .cyan,
                            text: LanguageManager.current.string("Uranus By The Numbers Temperature"))
                    
                    infoRow(icon: "hourglass", color: .teal,
                            text: LanguageManager.current.string("Uranus By The Numbers Age"))
                }
                .padding()
            ...
            .background(
                LinearGradient(
                    gradient: Gradient(colors: [Color.black, Color.cyan.opacity(0.3)]),
                    startPoint: .top,
                    endPoint: .bottom
                )
                .ignoresSafeArea()
    ...
    struct GalleriesView: View {
        ...
        let images = ["Uranus", "Uranus01", "Uranus02", "Uranus03", "Uranus04", "Uranus05"]
        
        var body: some View {
            ZStack {
                // 🌌 Cosmos Background
                LinearGradient(
                    gradient: Gradient(colors: [Color.black, Color.cyan.opacity(0.4), Color.blue.opacity(0.4)]),
                                    ...
                                    .overlay(
                                        RoundedRectangle(cornerRadius: 16)
                                            .stroke(LinearGradient(
                                                gradient: Gradient(colors: [.cyan, .blue, .clear]),
                                                startPoint: .topLeading,
                                                endPoint: .bottomTrailing
                                            ), lineWidth: 2)
                                    )
    ...
    struct MythView: View {
        ...
        private let myths: [Myth] = [
            Myth(culture: LanguageManager.current.string("Greek Myth Culture"), godName: LanguageManager.current.string("Uranus Myth God"), description: LanguageManager.current.string("Uranus Myth Description"), imageName: "UranusGod_Uranus"),
            Myth(culture: LanguageManager.current.string("Roman Myth Culture"), godName: LanguageManager.current.string("Roman Myth God"), description: LanguageManager.current.string("Roman Myth Description"), imageName: "UranusGod_Caelus"),
            Myth(culture: LanguageManager.current.string("Hindu Myth Culture"), godName: LanguageManager.current.string("Hindu Myth God"), description: LanguageManager.current.string("Hindu Myth Description"), imageName: "UranusGod_Varuna"),
            Myth(culture: LanguageManager.current.string("Maori Myth Culture"), godName: LanguageManager.current.string("Maori Myth God"), description: LanguageManager.current.string("Maori Myth Description"), imageName: "UranusGod_Rangi")
        ]
        
        var body: some View {
            ZStack {
                // Gradient background
                LinearGradient(gradient: Gradient(colors: [Color.cyan.opacity(0.8), Color.black]),
                               startPoint: .bottom,
                               endPoint: .top)
                    .ignoresSafeArea()
                ...
                ScrollView {
                    VStack(spacing: 24) {
                        // Header
                        VStack(spacing: 8) {
                            Text(LanguageManager.current.string("Uranus Myths Title"))
                                .font(.title.bold())
                                .foregroundColor(.white)
                            
                            Text(LanguageManager.current.string("Uranus Myths Description"))
                                .font(.body)
                                .foregroundColor(.white.opacity(0.8))
    ...
    struct InternalView: View {
        ...
        struct UranusLayer: Identifiable {
            let id = UUID()
            let name: String
            let description: String
        ...
        private let layers: [UranusLayer] = [
            UranusLayer(
                name: LanguageManager.current.string("Uranus Layer Core"),
                description: LanguageManager.current.string("Uranus Layer Core Description"),
                color: LinearGradient(colors: [.blue, .cyan], startPoint: .topLeading, endPoint: .bottomTrailing),
                icon: "circle.fill"
            ),
            UranusLayer(
                name: LanguageManager.current.string("Uranus Layer Mantle"),
                description: LanguageManager.current.string("Uranus Layer Mantle Description"),
                color: LinearGradient(colors: [.cyan, .white], startPoint: .topLeading, endPoint: .bottomTrailing),
                icon: "waveform.path.ecg"
            ),
            UranusLayer(
                name: LanguageManager.current.string("Uranus Layer Atmosphere"),
                description: LanguageManager.current.string("Uranus Layer Atmosphere Description"),
                color: LinearGradient(colors: [.cyan, .blue], startPoint: .top, endPoint: .bottom),
                icon: "cloud.fill"
            )
        ]
        ...
        var body: some View {
            ScrollView {
                VStack(spacing: 20) {
                    Text(LanguageManager.current.string("Uranus Internal Title"))
                        .font(.title2)
                        .fontWeight(.bold)
                        .foregroundColor(.white)
                        .padding(.top, 10)
                    ...
                    Image("Uranus")
                        ...
                        .clipShape(RoundedRectangle(cornerRadius: 12))
                        .padding()
                        .matchedGeometryEffect(id: "internal-uranus", in: animation)
                }
                .padding(.bottom, 30)
            }
            .background(
                LinearGradient(colors: [.black, .cyan.opacity(0.6)], startPoint: .top, endPoint: .bottom)
                    .ignoresSafeArea()
            )
        }
    }
            ...
            ScrollView {
                VStack(spacing: 20) {
                    ...
                    Image("Uranus")
                        ...
                        .shadow(radius: 8)
                        .padding(.horizontal)
                        .matchedGeometryEffect(id: "indepth-uranus", in: animation)
                    
                    // Title
                    Text(LanguageManager.current.string("Uranus In Depth Title"))
                        .font(.title2.bold())
                        .foregroundColor(.white)
                        .padding(.horizontal)
                    ...
                    VStack(spacing: 16) {
                        InfoCardView(
                            icon: "cloud.fill",
                            title: LanguageManager.current.string("Uranus In Depth Atmosphere"),
                            description: LanguageManager.current.string("Uranus In Depth Atmosphere Description"),
                            iconColor: .cyan
                        )
                        
                        InfoCardView(
                            icon: "moon.fill",
                            title: LanguageManager.current.string("Uranus In Depth Moons"),
                            description: LanguageManager.current.string("Uranus In Depth Moons Description"),
                            iconColor: .blue
                        )
                        
                        InfoCardView(
                            icon: "arrow.triangle.2.circlepath",
                            title: LanguageManager.current.string("Uranus In Depth Tilt"),
                            description: LanguageManager.current.string("Uranus In Depth Tilt Description"),
                            iconColor: .purple
                        )
                        
                        InfoCardView(
                            icon: "airplane",
                            title: LanguageManager.current.string("Uranus In Depth Missions"),
                            description: LanguageManager.current.string("Uranus In Depth Missions Description"),
                            iconColor: .green
                        )
                    }
                    .padding(.horizontal)
            ...
            ScrollView {
                VStack(spacing: 24) {
                    ...
                    ZStack(alignment: .bottomLeading) {
                        ...
                        Text(LanguageManager.current.string("Uranus Exploration Title"))
                    ...
                    VStack(alignment: .leading, spacing: 16) {
                        MissionCardView(
                            title: LanguageManager.current.string("Uranus Mission Voyager 2"),
                            description: LanguageManager.current.string("Uranus Mission Voyager 2 Description"),
                    ...
                    // ✨ Highlight Quote
                    Text(LanguageManager.current.string("Uranus Exploration Quote"))
                        .font(.title3.italic())
...
#Preview {
    UranusView(
        planet: PlanetModel(name: "Uranus", planetDescription: "The seventh planet from the Sun, known for its icy blue-green atmosphere.", planet_order: 7),
