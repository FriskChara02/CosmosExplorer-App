...
struct AlphaCentauriView: View {
    ...
    private let infoItems: [String] = [
        LanguageManager.current.string("Alpha Centauri Random Info 1"),
        LanguageManager.current.string("Alpha Centauri Random Info 2"),
        LanguageManager.current.string("Alpha Centauri Random Info 3"),
        LanguageManager.current.string("Alpha Centauri Random Info 4"),
        LanguageManager.current.string("Alpha Centauri Random Info 5")
    ]
    ...
    var body: some View {
        NavigationStack {
            VStack {
                ...
                HStack {
                    ...
                    Button(action: {
                        if selectedTab == LanguageManager.current.string("Wiki") {
                            UIPasteboard.general.string = "https://en.wikipedia.org/wiki/Alpha_Centauri"
                        } else {
                ...
                TabBarAlphaCentauriView(selectedTab: $selectedTab)

                // Nội dung tab
                VStack {
                    if selectedTab == LanguageManager.current.string("Overview") {
                        OverviewAlphaCentauriView(
                            star: star,
                        ...
                        GalleriesAlphaCentauriView(animation: animation)
                    } else if selectedTab == LanguageManager.current.string("Comment") {
                        CommentAlphaCentauriView(starId: starId).environmentObject(commentVM)
                    } else if selectedTab == LanguageManager.current.string("Wiki") {
                        VStack {
                            Text(LanguageManager.current.string("Wikipedia: Alpha Centauri Star"))
                                .font(.headline)
                                .foregroundColor(.white)
                                .padding()
                            WebView(url: URL(string: "https://en.wikipedia.org/wiki/Alpha_Centauri")!)
            ...
            .background(Image("AlphaCentauri_background").resizable().scaledToFill().ignoresSafeArea().overlay(Color.black.opacity(0.4)))
            .animation(.easeInOut(duration: 1.0), value: glowIntensity)
            .onAppear {
...
struct TabBarAlphaCentauriView: View {
...
struct OverviewAlphaCentauriView: View {
    let star: StarModel
    ...
    var body: some View {
        ZStack {
            RealityView { content in
                let alphaCentauriMesh = MeshResource.generateSphere(radius: 1.0)
                var alphaCentauriMaterial = PhysicallyBasedMaterial()
                do {
                    let textureResource = try await TextureResource(named: "Star_texture")
                    alphaCentauriMaterial.baseColor = .init(texture: .init(textureResource))
                    if let normalResource = try? await TextureResource(named: "Star_texture") {
                        alphaCentauriMaterial.normal = .init(texture: .init(normalResource))
                    }
                } catch {
                    print("Error loading texture: \(error)")
                    alphaCentauriMaterial.baseColor = .init(tint: .white)
                }
                
                alphaCentauriMaterial.emissiveColor = .init(color: .cyan)
                alphaCentauriMaterial.emissiveIntensity = 8.0
                alphaCentauriMaterial.roughness = 1.0
                alphaCentauriMaterial.metallic = 0.0
                
                let alphaCentauriEntity = ModelEntity(mesh: alphaCentauriMesh, materials: [alphaCentauriMaterial])
                alphaCentauriEntity.name = "AlphaCentauri"
                alphaCentauriEntity.position = [0, 0, 0]
                ...
                alphaCentauriEntity.addChild(flareEntity)
                ...
                content.add(alphaCentauriEntity)
                ...
                Task { @MainActor in
                    ...
                    while true {
                        ...
                        alphaCentauriEntity.transform.rotation = rotationY * rotationX
            ...
            .frame(height: 300)
            .padding()
            .matchedGeometryEffect(id: "alphacentauri-\(star.id)", in: animation)
            .gesture(
                DragGesture()
                    .onChanged { value in
                        isDragging = true
...
struct InformationAlphaCentauriView: View {
    let star: StarModel
    @Binding var glowIntensity: Float
    ...
    var body: some View {
        GeometryReader { geometry in
            VStack {
                RealityView { content in
                    let alphaCentauriMesh = MeshResource.generateSphere(radius: 0.5)
                    var alphaCentauriMaterial = PhysicallyBasedMaterial()
                    do {
                        let textureResource = try await TextureResource(named: "Star_texture")
                        alphaCentauriMaterial.baseColor = .init(texture: .init(textureResource))
                    ...
                    Task { @MainActor in
                        ...
                        while true {
                            ...
                            alphaCentauriEntity.transform.rotation = rotation
                            try? await Task.sleep(nanoseconds: 16_666_666)
                        }
                    }
                }
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 16) {
                        ...
                        Text(star.aboutDescription.isEmpty ? LanguageManager.current.string("Alpha Centauri About Description") : star.aboutDescription)
                            .font(.body)
                            .foregroundColor(.white)
                            .padding(.horizontal)
                            .transition(.opacity)
                        ...
                        HStack {
                            ...
                            Text(LanguageManager.current.string("Video"))
                                ...
                                .foregroundColor(.white)
                            Spacer()
                            NavigationLink(destination: VideoListAlphaCentauriView(videoURLs: star.videoURLs)) {
                        ...
                        LazyVGrid(columns: [
                            ...
                            ForEach(star.videoURLs.isEmpty ? [
                                "https://www.youtube.com/embed/7wLFpEPgnyo",
                                "https://www.youtube.com/embed/cfWZQ410d2E"
                        ...
                        VStack(spacing: 12) {
                            infoCard(icon: "circle.fill", color: .orange, title: LanguageManager.current.string("Radius"), value: star.radius.isEmpty ? LanguageManager.current.string("~1.223 solar radii (A)") : star.radius)
                            infoCard(icon: "sun.max.fill", color: .red, title: LanguageManager.current.string("Distance from Sun"), value: star.distanceFromSun.isEmpty ? LanguageManager.current.string("~4.37 light-years") : star.distanceFromSun)
                            infoCard(icon: "hourglass", color: .teal, title: LanguageManager.current.string("Age"), value: star.age.isEmpty ? LanguageManager.current.string("~4.85 billion years") : star.age)
                        }
                        .padding(.horizontal)
                    }
                    .padding(.vertical)
...
struct VideoListAlphaCentauriView: View {
    ...
    var body: some View {
        ScrollView {
            LazyVGrid(columns: [
                ...
                ForEach(videoURLs.isEmpty ? [
                    "https://www.youtube.com/embed/7wLFpEPgnyo",
                    "https://www.youtube.com/embed/cfWZQ410d2E",
                    "https://www.youtube.com/embed/XKbQuNF0zPo",
                    "https://www.youtube.com/embed/39VKA8vPXGg"
        ...
        .background(Image("AlphaCentauri_background").resizable().scaledToFill().ignoresSafeArea())
        .navigationTitle(LanguageManager.current.string("Videos"))
        .navigationBarTitleDisplayMode(.inline)
    }
}

struct GalleriesAlphaCentauriView: View {
    let animation: Namespace.ID
    let images = ["AlphaCentauri_background", "AlphaCentauri01", "AlphaCentauri02", "AlphaCentauri03", "AlphaCentauri04", "AlphaCentauri05", "AlphaCentauri06"]
...
struct CommentAlphaCentauriView: View {
...
#Preview {
    AlphaCentauriView(
        star: StarModel(name: "Alpha Centauri", starDescription: "The closest star system.", star_order: 2),
