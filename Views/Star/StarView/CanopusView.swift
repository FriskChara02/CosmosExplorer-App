...
struct CanopusView: View {
    ...
    private let infoItems: [String] = [
        ...
        LanguageManager.current.string("Canopus Random Info 2"),
        LanguageManager.current.string("Canopus Random Info 3"),
        LanguageManager.current.string("Canopus Random Info 4"),
        LanguageManager.current.string("Canopus Random Info 5")
    ]
    ...
    var body: some View {
        NavigationStack {
            VStack {
                ...
                HStack {
                    ...
                    Button(action: {
                        if selectedTab == LanguageManager.current.string("Wiki") {
                            UIPasteboard.general.string = "https://en.wikipedia.org/wiki/Canopus"
                        } else {
                ...
                TabBarCanopusView(selectedTab: $selectedTab)
                    ...
                    if selectedTab == LanguageManager.current.string("Overview") {
                        ...
                        VStack {
                            Text(LanguageManager.current.string("Wikipedia: Canopus Star"))
                                ...
                                .foregroundColor(.white)
                                .padding()
                            WebView(url: URL(string: "https://en.wikipedia.org/wiki/Canopus")!)
...
struct OverviewCanopusView: View {
    let star: StarModel
    ...
    var body: some View {
        ZStack {
            RealityView { content in
                let canopusMesh = MeshResource.generateSphere(radius: 1.0)
                var canopusMaterial = PhysicallyBasedMaterial()
                do {
                    let textureResource = try await TextureResource(named: "Star_texture")
                    canopusMaterial.baseColor = .init(texture: .init(textureResource))
                    if let normalResource = try? await TextureResource(named: "Star_texture") {
                        canopusMaterial.normal = .init(texture: .init(normalResource))
                    }
                } catch {
                    print("Error loading texture: \(error)")
                    canopusMaterial.baseColor = .init(tint: .white)
                }
                
                canopusMaterial.emissiveColor = .init(color: .white)
                canopusMaterial.emissiveIntensity = 2.0
                canopusMaterial.roughness = 1.0
                canopusMaterial.metallic = 0.0
                
                let canopusEntity = ModelEntity(mesh: canopusMesh, materials: [canopusMaterial])
                canopusEntity.name = "Canopus"
                canopusEntity.position = [0, 0, 0]
                ...
                canopusEntity.addChild(flareEntity)
                ...
                Task { @MainActor in
                    ...
                    while true {
                        ...
                        canopusEntity.transform.rotation = rotationY * rotationX
            ...
            .frame(height: 300)
            .padding()
            .matchedGeometryEffect(id: "canopus-\(star.id)", in: animation)
            .gesture(
                DragGesture()
                    .onChanged { value in
                        isDragging = true
...
struct InformationCanopusView: View {
    let star: StarModel
    @Binding var glowIntensity: Float
    ...
    var body: some View {
        GeometryReader { geometry in
            VStack {
                RealityView { content in
                    let canopusMesh = MeshResource.generateSphere(radius: 0.5)
                    var canopusMaterial = PhysicallyBasedMaterial()
                    do {
                        let textureResource = try await TextureResource(named: "Star_texture")
                        canopusMaterial.baseColor = .init(texture: .init(textureResource))
                    ...
                    Task { @MainActor in
                        ...
                        while true {
                            ...
                            canopusEntity.transform.rotation = rotation
                            try? await Task.sleep(nanoseconds: 16_666_666)
                        }
                    }
                }
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 16) {
                        ...
                        Text(star.aboutDescription.isEmpty ? LanguageManager.current.string("Canopus About Description") : star.aboutDescription)
                            .font(.body)
                            .foregroundColor(.white)
                            .padding(.horizontal)
                            .transition(.opacity)
                        ...
                        HStack {
                            ...
                            Text(LanguageManager.current.string("Video"))
                                ...
                                .foregroundColor(.white)
                            Spacer()
                            NavigationLink(destination: VideoListCanopusView(videoURLs: star.videoURLs)) {
                        ...
                        LazyVGrid(columns: [
                            ...
                            ForEach(star.videoURLs.isEmpty ? [
                                "https://www.youtube.com/embed/kr48Xelm4_k",
                                "https://www.youtube.com/embed/oIsuh1X-hqU"
                        ...
                        VStack(spacing: 12) {
                            infoCard(icon: "circle.fill", color: .orange, title: LanguageManager.current.string("Radius"), value: star.radius.isEmpty ? LanguageManager.current.string("~71 solar radii") : star.radius)
                            infoCard(icon: "sun.max.fill", color: .red, title: LanguageManager.current.string("Distance from Sun"), value: star.distanceFromSun.isEmpty ? LanguageManager.current.string("~310 light-years") : star.distanceFromSun)
                            infoCard(icon: "hourglass", color: .teal, title: LanguageManager.current.string("Age"), value: star.age.isEmpty ? LanguageManager.current.string("~10-20 million years") : star.age)
                        }
                        .padding(.horizontal)
                    }
                    .padding(.vertical)
...
struct VideoListCanopusView: View {
    ...
    var body: some View {
        ScrollView {
            LazyVGrid(columns: [
                ...
                ForEach(videoURLs.isEmpty ? [
                    "https://www.youtube.com/embed/kr48Xelm4_k",
                    "https://www.youtube.com/embed/oIsuh1X-hqU",
                    "https://www.youtube.com/embed/NW3Tfa7wOxc",
                    "https://www.youtube.com/embed/UqlPIE-9-T4"
        ...
        .background(Image("Canopus_background").resizable().scaledToFill().ignoresSafeArea())
        .navigationTitle(LanguageManager.current.string("Videos"))
        .navigationBarTitleDisplayMode(.inline)
    }
}

struct GalleriesCanopusView: View {
    let animation: Namespace.ID
    let images = ["Canopus_background", "Canopus01", "Canopus02", "Canopus03", "Canopus04", "Canopus05", "Canopus06"]
...
#Preview {
    CanopusView(
        star: StarModel(name: "Canopus", starDescription: "The second-brightest star.", star_order: 1),
