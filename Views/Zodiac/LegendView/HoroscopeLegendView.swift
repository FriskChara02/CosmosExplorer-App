...
struct HoroscopeLegendView: View {
        ...
        HStack(spacing: 16) {
            ZStack {
                Circle()
                    .fill(
                        LinearGradient(
                            gradient: Gradient(colors: [Color.brown.opacity(0.6), Color.gray.opacity(0.4)]),
                            startPoint: .topLeading,
                            endPoint: .bottomTrailing
                        )
                    )
            ...
            VStack(alignment: .leading, spacing: 6) {
                Text(LanguageManager.current.string("capricorn"))
                    .font(.custom("Audiowide", size: 32))
                    .foregroundStyle(
                        LinearGradient(
                            gradient: Gradient(colors: [.white, .brown.opacity(0.8)]),
                            startPoint: .leading,
                            endPoint: .trailing
                        )
                    )
                    .shadow(color: .brown.opacity(0.5), radius: 10)
        ...
        .background(
            ZStack {
                ...
                RoundedRectangle(cornerRadius: 20)
                    .fill(
                        LinearGradient(
                            gradient: Gradient(colors: [Color.brown.opacity(0.15), Color.gray.opacity(0.1)]),
                            startPoint: .topLeading,
                            endPoint: .bottomTrailing
                        )
                    )
                ...
                ZStack {
                    if selectedTab == index {
                        RoundedRectangle(cornerRadius: 14)
                            .fill(
                                LinearGradient(
                                    gradient: Gradient(colors: [Color.brown.opacity(0.8), Color.gray.opacity(0.6)]),
                                    startPoint: .topLeading,
                                    endPoint: .bottomTrailing
                                )
                            )
                            .matchedGeometryEffect(id: "tab", in: animation)
                            .shadow(color: .brown.opacity(0.5), radius: 8, y: 4)
                    }
                }
            )
        }
    ...
    private var profileSection: some View {
        VStack(spacing: 12) {
            infoCard(title: LanguageManager.current.string("basic_info")) {
                infoRow(icon: "number", title: LanguageManager.current.string("zodiac_order"), value: LanguageManager.current.string("10th_zodiac"))
                infoRow(icon: "person.2", title: LanguageManager.current.string("nature"), value: LanguageManager.current.string("leader_nature"))
                infoRow(icon: "calendar", title: LanguageManager.current.string("weekday"), value: LanguageManager.current.string("saturday"))
                infoRow(icon: "sparkles", title: LanguageManager.current.string("element"), value: LanguageManager.current.string("earth"))
            }
            
            infoCard(title: LanguageManager.current.string("astronomy")) {
                infoRow(icon: "sparkle", title: LanguageManager.current.string("ruling_planet"), value: LanguageManager.current.string("saturn"))
                infoRow(icon: "figure.wave", title: LanguageManager.current.string("symbol"), value: LanguageManager.current.string("sea_goat"))
            }
            
            infoCard(title: LanguageManager.current.string("gemstones")) {
                Text(LanguageManager.current.string("capricorn_gems"))
                    .font(.system(size: 14))
            ...
            infoCard(title: LanguageManager.current.string("colors_symbols")) {
                infoRow(icon: "paintpalette", title: LanguageManager.current.string("colors"), value: LanguageManager.current.string("dark_colors"))
                infoRow(icon: "leaf", title: LanguageManager.current.string("flowers"), value: LanguageManager.current.string("gardenia_violet"))
                infoRow(icon: "cube", title: LanguageManager.current.string("metal"), value: LanguageManager.current.string("lead"))
                infoRow(icon: "hare", title: LanguageManager.current.string("animals"), value: LanguageManager.current.string("crocodile_sheep"))
            }
            
            infoCard(title: LanguageManager.current.string("health_relations")) {
                infoRow(icon: "heart.text.square", title: LanguageManager.current.string("body_parts"), value: LanguageManager.current.string("skin_bones_knees"))
                infoRow(icon: "heart.circle", title: LanguageManager.current.string("dating_with"), value: LanguageManager.current.string("taurus_virgo"))
                ...
                infoRow(icon: "checkmark.seal", title: LanguageManager.current.string("compatible_with"), value: LanguageManager.current.string("taurus_virgo_capricorn_pisces"))
                infoRow(icon: "xmark.seal", title: LanguageManager.current.string("clash_with"), value: LanguageManager.current.string("cancer"))
                infoRow(icon: "number.circle", title: LanguageManager.current.string("lucky_numbers"), value: LanguageManager.current.string("6_8_9"))
            }
        }
    }
    ...
    private var legendSection: some View {
        VStack(spacing: 16) {
            legendCard(
                ...
                title: LanguageManager.current.string("babylon_god"),
                content: LanguageManager.current.string("ea_sea_goat")
            )
            
            legendCard(
                icon: "person.fill",
                title: LanguageManager.current.string("greek_myth"),
                content: LanguageManager.current.string("pan_hermes")
            )
            
            legendCard(
                icon: "wave.3.right",
                title: LanguageManager.current.string("symbol_meaning"),
                content: LanguageManager.current.string("nam_duong_nile")
            )
        }
    }
    ...
    private var personalitySection: some View {
        VStack(spacing: 16) {
            traitCard(
                ...
                title: LanguageManager.current.string("ambitious_determined"),
                content: LanguageManager.current.string("ambitious_desc")
            )
            
            traitCard(
                icon: "shield",
                gradient: [Color.gray, Color.black],
                title: LanguageManager.current.string("disciplined_patient"),
                content: LanguageManager.current.string("disciplined_desc")
            )
            
            traitCard(
                icon: "lock",
                gradient: [Color.purple, Color.gray],
                title: LanguageManager.current.string("reserved_principled"),
                content: LanguageManager.current.string("reserved_desc")
            )
            
            traitCard(
                icon: "target",
                gradient: [Color.green, Color.brown],
                title: LanguageManager.current.string("goal_oriented"),
                content: LanguageManager.current.string("goal_desc")
            )
            
            traitCard(
                icon: "brain",
                gradient: [Color.blue, Color.purple],
                title: LanguageManager.current.string("logical_practical"),
                content: LanguageManager.current.string("logical_desc")
            )
            
            traitCard(
                icon: "person.3",
                gradient: [Color.teal, Color.gray],
                title: LanguageManager.current.string("responsible_leader"),
                content: LanguageManager.current.string("responsible_desc")
            )
        }
    }
    ...
    private var loveSection: some View {
        VStack(spacing: 16) {
            loveCard(
                icon: "heart.fill",
                title: LanguageManager.current.string("cautious_love"),
                content: LanguageManager.current.string("cautious_love_desc")
            )
    ...
    private var careerSection: some View {
        VStack(spacing: 16) {
            careerCard(
                ...
                title: LanguageManager.current.string("career"),
                content: LanguageManager.current.string("career_desc")
            )
        ...
        .background(
            ZStack {
                ...
                RoundedRectangle(cornerRadius: 16)
                    .stroke(
                        LinearGradient(
                            gradient: Gradient(colors: [Color.brown.opacity(0.3), Color.gray.opacity(0.2)]),
                            startPoint: .topLeading,
                            endPoint: .bottomTrailing
                        ),
                        lineWidth: 1
        ...
        .background(
            ZStack {
                RoundedRectangle(cornerRadius: 16)
                    .fill(
                        LinearGradient(
                            gradient: Gradient(colors: [Color.brown.opacity(0.15), Color.orange.opacity(0.1)]),
                            startPoint: .topLeading,
                            endPoint: .bottomTrailing
                        )
                    )
